...
for (const vertex of graph) {
    const createdLink = tree.union(vertex[0], vertex[1])
    ...
    for (const key of tree.root.keys()) {
        const root = tree.findRoot(key)
        obj[root] = (obj[root] ?? 0) + 1
    }

    if (Object.keys(obj).length === 1) {
        const [x1] = vertex[0].split(',')
        const [x2] = vertex[1].split(',')
        console.log(+x1 * +x2)
        break
    }

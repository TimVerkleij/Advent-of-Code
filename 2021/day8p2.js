let input = `dbc gfecab afcdg dfebcag bd dgbe bcaeg dcefab ecgadb agcbd | acdgb gbcda gdecfba bacge
...
let lengthsAndValues = { '2': [1], '3': [7], '4': [4], '6': [0, 6, 9], '5': [2, 3, 5], '7': [8] }
let allSegments = "abcdefg"
let total = 0

for (let i = 0; i < input.length; i++) {
    const signal = input[i][0]
    const output = input[i][1].split(" ").map(value => {return value.split("").sort().join("")})
    let signals = signal.split(" ")
    let actualSignals = {}
    let unknownNumbers = {}
    let top, mid, bot, topL, topR, botL, botR = ""
    for (let j = 0; j < signals.length; j++) {
        const number = signals[j];
        let possibleNumbers = lengthsAndValues[number.length]
        if (possibleNumbers.length === 1) {
            actualSignals[possibleNumbers[0]] = number
        } else {
            unknownNumbers[number] = possibleNumbers
        }
        // console.log(actualSignals)
    }
    top = actualSignals[7].split("").filter(letter => { return !actualSignals[1].includes(letter) })[0]

    // console.log(unknownNumbers)
    // Object.entries(unknownNumbers).forEach(
    //     ([key, value]) => {
    //         if(key.length === 6) {
    //             console.log(key)
    //         }
    //     }
    // );
    let one = actualSignals[1]
    let nine = Object.entries(unknownNumbers).map(
        ([key, value]) => {
            if (key.length === 6) {
                return key
            }
    ...
    bot = nine.split("").filter(value => {
        let four = actualSignals[4]
        let total = 0
        return !four.includes(value) && value !== top
    })[0]

    botL = allSegments.split("").filter(value => {
        return !nine.includes(value)
    })[0]

    let six, zero

    Object.entries(unknownNumbers).map(
        ([key, value]) => {
            if (key.length === 6) {
                return key
            }
        ...
        return value.includes(top) && value.includes(bot) && value.includes(botL)
    }).forEach(value => {
        if ((value.includes(one[0]) ^ value.includes(one[1]))) {
            six = value
        } else {
            zero = value
        }
    })
    // console.log(nine, bot, top, botL)
    // console.log(" ")
    mid = allSegments.split("").filter(value => {
        return !zero.includes(value)
    })[0]



    topR = allSegments.split("").filter(value => {
        return !six.includes(value)
    })[0]
    ...
    // console.log(zero, one, six, nine)
    // console.log(top, mid, bot, botL)
    ...
    for (let j = 0; j < output.length; j++) {
        ...
        if (number.length === 2) {
            ...
            if (number == zero) {
                ...
                output[j] = 9
            } else if (number.includes(botL)) {
                output[j] = 2
            } else if (number.includes(topR)) {
                output[j] = 3
            } else {
                output[j] = 5
            }

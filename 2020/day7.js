...
input.forEach(bag => {
    ...
    let contents = bag.split(" contain ")[1].split(", ")
    bagObj[container] = contents
});

let bagToFind = "shiny gold bag"
let bagsWithBagToFind = new Set()

findBag = (bag) => {
    Object.keys(bagObj).forEach(key => {
        bagObj[key].forEach(value => {
            if (value.includes(bag)) {
                bagsWithBagToFind.add(key)
                bagFound = true
            }
        })
    })
...
bagsWithBagToFind.forEach(bag => {
    findBag(bag)
});

console.timeEnd("timer")

console.log(bagsWithBagToFind.size)

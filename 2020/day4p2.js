...
function loop(value, index, array) {
    ...
    for (let i = 0; i < fields.length; i++) {
        ...
        if (field.includes("\n")) {
            ...
            splitted.forEach((val) => { // console.log(val, 6)
                passport.push(val)
            })
        } else { // console.log(field, 5)
            passport.push(field)
        }
...
passports.forEach((value, index, array) => {
    let validFields = 0

    for (let i = 0; i < value.length; i++) { // console.log(validFields)
        let field = value[i];
        let keyValue = field.split(":")[1]
        if (field.includes("byr")) {
            if (keyValue >= 1920 && keyValue <= 2002) {
                validFields++
            }
        }
        if (field.includes("iyr")) {
            if (keyValue >= 2010 && keyValue <= 2020) {
                validFields++
            }
        }
        if (field.includes("eyr")) {
            if (keyValue >= 2020 && keyValue <= 2030) {
                validFields++
            }
        }
        if (field.includes("hgt")) {
            if(keyValue.includes("cm")){
                let height = keyValue.split("cm")[0]
                if(height >= 150 && height <= 193){
                    validFields++
                }
            } else if(keyValue.includes("in")){
                let height = keyValue.split("in")[0]
        ...
        if (field.includes("hcl")) {
            if(keyValue.match(/#[0-9a-f]{6}$/) !== null){
                validFields++
            }
        }
        if (field.includes("ecl")) {
            if(hairColors.includes(keyValue)){
                validFields++
            }
        }
        if (field.includes("pid")) {
            if(keyValue.match(/[0-9]{9}$/) && keyValue.length == 9){
                validFields++
            }
        }
        if (validFields === 7) { // console.log(value, "valid!")
            if (! allValids.includes(value)) {
                allValids.push(value)
                totalValidPassports++
            }
        }
    }

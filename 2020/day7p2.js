input = `wavy bronze bags contain 5 striped gold bags, 5 light tomato bags.
...
input.forEach(bag => {
    let container = bag.split(" contain ")[0].slice(0, -1)
    let contents = bag.split(" contain ")[1].split(", ").map(b => {
        if (b.endsWith("s")) {
            return b.slice(0, -1)
        } else {
            return b
    ...
    bagObj[container] = contents
});


let num = 1
let bagToFind = "shiny gold bag"
let bagsWithBagToFind = []
bagTotal = 0

findBag = (bag, num) => {
    Object.keys(bagObj).forEach(key => {
        let currentBag = bagObj[key]

        if (key === bag) {
            for (let i = 0; i < currentBag.length; i++) {
                let element = currentBag[i];

                if (element === "no other bag") {
                    continue;
                }

                let amount = parseInt(element.slice(0, 1)) * num
                let bagType = element.slice(2)
                bagsWithBagToFind.push([amount, bagType])
                bagTotal = bagTotal + amount
            }
        }
    })

    bagsWithBagToFind.forEach(currentBag => {
        let number = currentBag[0]
        let thisBag = currentBag[1]
        let index = bagsWithBagToFind.indexOf(currentBag)
        bagsWithBagToFind.splice(index, 1)
        findBag(thisBag, number)
    })

}

findBag(bagToFind, num)

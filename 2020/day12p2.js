input = `R90
F56
R90
F56
R90
R180
W5
L90
E2
L90
S5
E1
F11
L90
F46
S2
E2
S1
E2
E3
N4
W1
N4
L90
W4
L90
N1
E1
S4
F45
S4
W4
S1
E3
E3
E1
F76
S5
E1
N3
W2
F55
E3
F34
S2
S2
E3
S5
L90
L90
W5
F23
R90
F14
F21
L90
N3
L180
N3
F98
R180
S5
F94
R90
N5
L90
S3
F85
R90
F77
S1
R90
E3
S2
E5
R180
W3
F46
R90
F44
S3
W3
L180
N4
L90
N1
E2
L90
S4
R90
E1
N4
F100
L90
S4
F94
L180
W2
S1
R90
E5
L90
S4
L90
W3
E1
R270
S4
E2
N5
F3
R180
F21
L90
F57
W1
F77
F24
N4
E5
S3
L90
F64
N5
L90
N1
F42
S5
W1
F50
S1
W3
N5
E5
F19
R180
W4
S4
F20
L90
W1
F88
R90
F57
L90
N4
W5
R90
S3
E2
S1
E2
N1
E4
S1
F3
R90
W1
N4
R90
F38
E5
E1
N2
R90
N5
S4
E3
F8
W5
R90
E4
F19
N1
F50
E1
F54
W5
L90
F10
S2
E3
R90
S5
L90
F59
N3
R90
W5
F33
N4
R270
F46
S2
L270
F97
E4
N2
E5
R90
F31
R180
N1
W1
R90
F40
R90
S5
F15
L90
E2
L90
L90
F53
N4
F28
E5
F58
L90
W1
L90
E1
L90
N1
F42
S2
S3
F62
E5
F73
W1
R270
F10
R90
E5
N3
E4
S1
E2
N4
E4
F47
E3
E5
S5
F24
L90
N2
L180
S2
F82
W3
S2
W1
R90
N5
E3
N4
L90
N1
F53
S3
L90
S5
F88
F40
S1
E4
N4
F66
W3
L90
E1
R180
F73
R90
W4
N4
F85
E4
F66
S1
W4
F16
R180
N5
L90
S3
E4
N4
W2
L90
R90
W3
N5
R90
F45
E2
S5
R90
F6
W4
S2
E4
F74
W4
L90
F75
W2
L90
F97
R90
N4
F27
L180
F45
S1
L90
W2
F10
W3
S4
F42
W1
F30
R90
E2
F94
E5
R90
F41
E1
N3
W1
N2
R90
S5
F93
N1
R90
F38
S3
F50
L270
W1
S4
F64
W1
N1
F61
R90
W2
R90
N1
F31
S2
F7
N2
R90
S5
E3
S1
E1
R90
E2
R90
E4
F76
R90
W4
F15
S3
F2
R90
N4
F66
L180
W3
N2
F1
S4
L90
N1
F19
F20
E1
R90
N3
F54
N4
F62
W2
S1
F49
W3
S4
W3
N2
E4
N1
R90
E1
N2
L90
F46
S4
E2
L90
S3
L90
N4
R180
F56
L90
E2
F51
W1
N5
E2
L90
E3
S5
W3
F73
E5
F100
L90
N2
W2
L180
F89
L90
N3
W4
S2
W1
L270
E5
N2
R90
F65
W1
F41
E2
N4
E1
F73
L180
S4
L90
N4
W3
N4
E1
R180
F67
E5
L90
N5
F75
N3
E4
R90
F28
R90
S3
F77
L180
N2
N1
R90
F77
S3
W3
N4
F60
W4
F40
L90
W3
W3
E1
S2
W2
N1
F86
R180
E1
L180
F17
R90
F11
E1
S2
F48
S2
W2
F1
N5
F53
N2
W1
L180
L90
N5
W1
L180
E1
F67
W4
N3
E5
N1
F88
R90
F93
E1
F33
L90
F1
W2
N4
W3
F85
S1
E2
F69
R90
S4
W2
L90
W3
S5
F14
W1
S5
W2
F95
F30
E1
F4
W3
R90
F93
N5
E1
F9
L90
W5
L270
E1
L90
S2
F97
S1
F18
E3
R90
N1
R90
N5
F72
S4
L90
W3
S3
F13
W5
N2
F31
F17
S4
E2
L90
N5
R90
F42
W2
N1
S3
F60
W5
N4
R90
E5
S3
F88
E4
N5
W3
L180
N3
F66
E5
R270
E3
L180
W4
W3
S2
W1
F69
E5
L90
F23
W5
L90
F49
S3
R180
W2
L90
N1
R90
S4
F71
N5
F36
R180
F62
L180
F39
N5
F36
R180
N4
W4
E5
F94
F39
L90
N1
F70
R90
W2
R180
F16
E3
S1
R180
E1
S3
E5
L90
N5
F76
N4
R90
E3
L180
N3
F20
S3
R90
N3
L270
E4
N5
F49
S3
F79
W3
R270
E1
L180
R90
F34
W3
S5
E5
F76
N3
N5
F66
L90
F48
E1
L270
E3
S2
L180
N3
F1
W5
L90
F9
S5
F64
L180
W1
N1
E2
F4
R90
F81
W5
S4
E1
N4
E1
F61
R180
S2
R180
F24
N3
W5
N1
F67
L180
F50
W5
R90
N2
L180
F80
W5
L90
N1
F31
E5
R90
N5
R90
F93
E2
E3
L90
E2
L90
E1
N4
W4
F80
R180
F84
E5
N1
F37
S3
W5
S4
F9
R90
E2
R90
F39
R90
F77
N3
R90
E5
N2
F49
S2
W3
F42
N1
F19
E2
N3
R90
F48
N1
W3
S2
E3
L90
E1
R90
F100
E2
L90
F90
W1
F66
N4
R180
W2
F33`

// input = `F10
// N3
// F7
// R90
// F11`

input = input.split("\n")

const {PerformanceObserver, performance} = require('perf_hooks');

let t0 = performance.now()


let cords = {
    x: 0,
    y: 0
}
let waypoint = {
    x: 10,
    y: 1
}
let newWaypoint = {
    x: 0,
    y: 0
}
let facing = 90

for (let i = 0; i < input.length; i++) {
    let action = input[i];
    amount = Number(action.slice(1))
    action = action.slice(0, 1)


    // console.log(action, amount)


    if (action === "F") {
        moveBoat(amount)
    } else if (action === "N") {
        moveCheckpoint(0, amount)
    } else if (action === "E") {
        moveCheckpoint(90, amount)
    } else if (action === "S") {
        moveCheckpoint(180, amount)
    } else if (action === "W") {
        moveCheckpoint(270, amount)
    } else if (action === "R") {
        let rotationTimes = amount / 90
        for (let j = 0; j < rotationTimes; j++) {

            if (waypoint["y"] >= 0) {
                newWaypoint["x"] = waypoint["y"]
            } else if (waypoint["y"] < 0) {
                newWaypoint["x"] = waypoint["y"]
            }
            if (waypoint["x"] >= 0) {
                newWaypoint["y"] = waypoint["x"] * -1
            } else if (waypoint["x"] < 0) {
                newWaypoint["y"] = waypoint["x"] * -1
            }
            waypoint["x"] = newWaypoint["x"]
            waypoint["y"] = newWaypoint["y"]
        }
    } else if (action === "L") {
        let rotationTimes = amount / 90
        for (let j = 0; j < rotationTimes; j++) {
            if (waypoint["y"] >= 0) {
                newWaypoint["x"] = waypoint["y"] * -1
            } else if (waypoint["y"] < 0) {
                newWaypoint["x"] = waypoint["y"] * -1
            }
            if (waypoint["x"] >= 0) {
                newWaypoint["y"] = waypoint["x"]
            } else if (waypoint["x"] < 0) {
                newWaypoint["y"] = waypoint["x"]
            }
            waypoint["x"] = newWaypoint["x"]
            waypoint["y"] = newWaypoint["y"]
        }
    }

}

function moveCheckpoint(direction, steps) {
    if (direction % 360 === 0) {
        waypoint["y"] = waypoint["y"] + steps
    } else if (direction === 90) {
        waypoint["x"] = waypoint["x"] + steps
    } else if (direction === 180) {
        waypoint["y"] = waypoint["y"] - steps
    } else if (direction === 270) {
        waypoint["x"] = waypoint["x"] - steps
    } else {
        console.log(direction, steps, cords)
    }
}

function moveBoat(steps) {
    let x = waypoint["x"] * steps
    let y = waypoint["y"] * steps
    cords["x"] = cords["x"] + x
    cords["y"] = cords["y"] + y
}

if (cords["x"] < 0) {
    cords["x"] = cords["x"] * -1
}
if (cords["y"] < 0) {
    cords["y"] = cords["y"] * -1
}

let t1 = performance.now()

console.log(cords["x"] + cords["y"])

console.log(t1 - t0 + " ms")
